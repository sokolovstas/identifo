...
func (ar *Router) FederatedLogin() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			ar.Error(w, ErrorAPIRequestAppIDInvalid, http.StatusBadRequest, "Can't get App by ID", "FederatedLogin.AppByID")
			return
		}
		...
		for k, p := range app.FederatedProviders {
			if provider, ok := model.FederatedProviders[k]; ok {
				params := []interface{}{
					p.Params,
					// callbackURL
					redirect + "?appId=" + appId + "&provider=" + k,
				}
...
func (ar *Router) FederatedLoginComplete() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		value, err := getFromSession(getSessionName(appId, providerName), r)
		if err != nil {
		...
		if err == nil {
			user, err := ar.userStorage.UserByFederatedID(providerName, gothUser.UserID)

			if err == model.ErrUserNotFound && gothUser.Email != "" {
				user, err = ar.userStorage.UserByEmail(gothUser.Email)
				if err == nil {
					user.AddFederatedId(providerName, gothUser.UserID)
					ar.userStorage.UpdateUser(user.ID, user)
				}
			}

			if err == model.ErrUserNotFound && !app.RegistrationForbidden {
				user, err = ar.userStorage.AddUserWithFederatedID(model.User{
...
func GetAuthURL(res http.ResponseWriter, req *http.Request) (string, error) {
	...
	err = storeInSession(getSessionName(appId, providerName), fsess.Marshal(), req, res)
...
var CompleteUserAuth = func(res http.ResponseWriter, req *http.Request) (goth.User, error) {
	...
	value, err := getFromSession(getSessionName(appId, providerName), req)
	if err != nil {
		return goth.User{}, err
	}
...
func getAppId(req *http.Request) (string, error) {
	if a := req.URL.Query().Get("appId"); a != "" {
		return a, nil
	}

	// if not found then return an empty string with the corresponding error
	return "", errors.New("you must select a appId")
}
